          ...
          {todos.map((todo) => (
            <li key={todo._id} className="border-b py-2">
              <div className="flex items-center justify-between">
                <span>{todo.body}</span>
                <div>
                  <button onClick={() => markComplete(todo._id)} className={`mx-2 py-1 px-3 rounded-md ${todo.completed ? 'text-green-500' : 'text-red-500'} text-white`}
                    disabled={todo.completed}>
                    {todo.completed ? <FaCheck /> : <FaBan />}
                  </button>
